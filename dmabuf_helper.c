...
int create_udmabuf(size_t size, struct dmabuf_info *info) {
    ...
    // メモリをマップ
    info->mapped_addr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, info->fd, 0);
    ...
    printf("udmabuf created successfully: fd=%d, size=%zu, mapped=%p\n", 
           info->fd, info->size, info->mapped_addr);
    
    return 0;
}

// dmabufをネットワークデバイスにバインド（RX）
int bind_dmabuf_rx(const char *ifname, int ifindex, int queue_idx, struct dmabuf_info *info) {
    struct nl_sock *sk;
    struct nl_msg *msg;
    int family_id, err = -1;
...
int main(int argc, char *argv[]) {
    ...
    printf("RX dmabuf: fd=%d, size=%zu, mapped=%p\n", 
           rx_dmabuf.fd, rx_dmabuf.size, rx_dmabuf.mapped_addr);
    printf("TX dmabuf: fd=%d, size=%zu, mapped=%p\n", 
           tx_dmabuf.fd, tx_dmabuf.size, tx_dmabuf.mapped_addr);
